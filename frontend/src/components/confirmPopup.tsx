...
interface Props {
  ...
  type?: string
  input?: string
  submitedConfirm?: boolean
  setSubmitedConfirm?: any
  inputValue?: string
  setInputValue?: any
}
...
const ConfirmPopup: FC<Props> = ({
  ...
  const cancelHandler = (passedType: string) => {
    if (passedType === 'rank') {
      ...
      if (setSubmitedConfirm) setSubmitedConfirm(false)
  ...
  const confirmHandler = (passedType: string) => {
    if (passedType === 'rank') {
      ...
      if (inputValue && inputValue.length) {
        ...
        if (setSubmitedConfirm) setSubmitedConfirm(true)
      }
    } else if (passedType === 'deleteDiscount') {
          ...
          >
            ...
            >
              ...
              <div className='actions'>
                <button
                  onClick={() => cancelHandler(type ? type : '')}
                  className='cancel'
                >
                  Cancel
                </button>
                <button
                  onClick={() => confirmHandler(type ? type : '')}
                  className='confirm'
                >
                  Confirm
                </button>
