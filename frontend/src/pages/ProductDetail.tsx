...
const ProductDetail: FC<Props> = ({ cartCount, setCartCount }) => {
  ...
  let matched = savedCart
    ? savedCart.find((each: any) => each._id === id)
    : undefined
  ...
  const addToCart = () => {
    const match = cartItems
      ? cartItems.find((each: any) => each._id === id)
      : false
              ...
              {product.countInStock !== 0 && (
                <>
                  ...
                  {product.countInStock <= 6 && (
                    <span className='onlyStock mobile'>
