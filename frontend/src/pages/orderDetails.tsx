...
const OrderDetails = () => {
  const orderThatIsPlaced = useSelector((state: any) => state.order)
  useEffect(() => {
  ...
  const { orderPayLoading, success } = useSelector(
  ...
  const qrScanHandler = (result: any) => {
    if (result) {
      setQrResult(result)
    }
  }
  const [showSVGAnimation, setShowSVGAnimation] = useState<any>(null)
  ...
  const qrErrorHandler = (result: any) => {
    if (result) {
      console.log(result)
    }
  }
