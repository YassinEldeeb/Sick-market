...
const Cart: FC<Props> = ({ cartCount, setCartCount }) => {
  ...
  useEffect(() => {
    if (check) {
      ...
      cartItems.forEach((product: any) => {
        removed.push(...check.removed.filter((e: any) => e === product._id))
        sold.push(...check.soldOut.filter((e: any) => e === product._id))
      })
      setRemovedProducts([...removed, ...sold] as any)
    }
  }, [check])
  ...
  useEffect(() => {
    if (checkProductsSuccess)
      setCartCount(
        qtyArr.length ? qtyArr.reduce((acc: any, item: any) => acc + item) : 0
      )
          ...
          {cartItems.length > 0 && (
            <>
              <div className='subtotal-mobile'>
                <h3>
                  Subtotal (
                  {qtyArr.reduce((acc: number, item: number) => acc + item)}{' '}
                  items):
                </h3>
                <h4>
                  {pricesArr
              ...
              {pricesArr.reduce((acc: number, price: number) => acc + price) +
                pricesArr.reduce((acc: number, price: number) => acc + price) *
                  (14 / 100) >=
                2000 && (
              ...
              {pricesArr.reduce((acc: number, price: number) => acc + price) <
                2000 && <li className='removeMargin'></li>}
              <Link
                id={`${
