...
const DashboardOrderActions: FC<Props> = ({ scrolled }) => {
  ...
  const { orders } = useSelector((state: any) => state.dashboardOrders)
  ...
  const {
    ...
    confirm,
    rejectLoading,
  } = useSelector((state: any) => state.orderActions)
  ...
  useEffect(() => {
    if (order && inView && !mapState && mapboxMapRef.current) {
      const container = mapboxMapRef.current!
