...
const Payment = () => {
  ...
  const pricesArr = !isBuyNow
    ? cartItems.map((each: any) => each.price * each.qty)
    : product.price * product.qty

  const totalPrice = !isBuyNow
    ? pricesArr.length
      ? pricesArr.reduce((acc: number, item: number) => acc + item)
      : null
    : pricesArr
  ...
  const [discount2, setDiscount2] = useState<any>(false)
  const [code, setCode] = useState(discount ? discount.code.code : '')

  const applyCodeHandler = (e: any) => {
    e.preventDefault()
...
const StyledPayment = styled.div`
  ...
  .couponCont {
    ...
    .Indication {
      ...
      &.error {
        background: url(${(props: any) => props.img2});
        background-size: cover;
      }
      #loader:first-child {
        position: absolute;
