...
const DashboardOrders: FC<Props> = ({ skip, setSkip }) => {
  ...
  useEffect(() => {
    ...
    if (!location.pathname.split('/')[3] && size(searches) === 0) {
  ...
  const { orders, loading, count, error, filtering, infiniteLoading } =
    useSelector((state: any) => state.dashboardOrders)
  const { loading: orderLoading } = useSelector(
    (state: any) => state.orderActions
  )
  ...
  const selectHandler = (e: any, bunch?: any) => {
    ...
    if (!bunch) {
      if (e.length > 0) {
        if (e.length > filter.length) {
          ...
          const removed: any = filter.filter((x) => !e.includes(x))[0]
          ...
          const insert = function (arr: any, index: any, item: any) {
            arr.splice(index, 0, item)
          }

          insert(copy, position === 1 ? index + 1 : position, {
            label: value,
            value,
          })
        }
  ...
  useEffect(() => {
    if (size(searches) > 0 && !location.pathname.split('/')[3]) {
      dispatch(
