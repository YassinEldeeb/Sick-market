...
const NotFound = () => {
  ...
  window.addEventListener('mousemove', (e) => {
    if (torch.current) {
      const torchElement = torch.current! as HTMLElement
      torchElement.style.top = `${e.pageY}px`
      torchElement.style.left = `${e.pageX}px`
    }
  })
  window.addEventListener('touchmove', (e) => {
    if (torch2.current) {
      const torchElement = torch2.current! as HTMLElement
      torchElement.style.top = `${e.touches[0].clientY}px`
      torchElement.style.left = `${e.touches[0].clientX}px`
    }
  })
  return (
...
const StyledNotFound = styled.div<any>`
  cursor: none;
