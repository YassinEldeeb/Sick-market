import React, { useState, useEffect, useRef, lazy, Suspense, FC } from 'react'
...
interface Props {
  setDashboardScrollPosition?: number
  dashboardScrollPosition?: React.Dispatch<React.SetStateAction<number>>
}
...
const Dashboard: FC<Props> = ({
  ...
  const { loading: orderLoading } = useSelector(
    (state: any) => state.dashboardOrders
  )
  const { loading: discountsLoading } = useSelector(
  ...
  const dashboardUsers = useSelector((state: any) => state.dashboardUsers)
  const userActions = useSelector((state: any) => state.userActions)
  const { user } = useSelector((state: any) => state.userInfo)
  const productList = useSelector((state: any) => state.productList)
  const dashboardOrders = useSelector((state: any) => state.dashboardOrders)
  const dashboardDiscounts = useSelector((state: any) => state.discounts)
  ...
  useEffect(() => {
    ...
    if (location.pathname.split('/')[3] === 'add') {
      const firstChild = document.querySelector('.view') as HTMLDivElement
    ...
    <StyledDashboard>
      ...
      <Scrollbars
        ...
        renderView={(props: any) => (
          <div {...props} className='view' id='view' />
        )}
