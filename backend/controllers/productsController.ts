...
const checkProducts = asyncHandler(async (req: any, res) => {
  ...
  for (let i = 0; i < products.length + 1; i++) {
    const e = products[i]
    if (e) {
      const product: any = await Product.findById(e)
    ...
    if (i === products.length) {
      productsArr.sort((a: any, b: any) => b.countInStock - a.countInStock)
...
const deleteProduct = asyncHandler(async (req: any, res) => {
  ...
  if (product) {
    console.log(join(__dirname, '../../', product.image))
    if (
...
const addProduct = asyncHandler(async (req: any, res) => {
  const {
    name,
    brand,
    category,
...
const updateProduct = asyncHandler(async (req: any, res) => {
  const product: any = await Product.findById(req.params.id).populate(
    'user',
    'name'
  )
...
const resizeProductImage = asyncHandler(async (req: any, res) => {
  const product: any = await Product.findById(req.params.id)
...
const getTinyProductImage = asyncHandler(async (req: any, res) => {
  const product: any = await Product.findById(req.params.id)
...
const searchProductsSuggesstions = asyncHandler(async (req: any, res) => {
